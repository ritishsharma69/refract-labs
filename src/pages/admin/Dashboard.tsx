...
import { Users, Briefcase, MessageSquare, Star, TrendingUp, Activity, ArrowRight, CheckCircle } from 'lucide-react';
import { Link } from 'react-router-dom';
import { fetchTeamMembers, fetchWorkItems, fetchTestimonialItems, subscribeToContentUpdates } from '../../lib/content-store';
...
type Counts = { team: number; works: number; featuredWorks: number; testimonials: number; featuredTestimonials: number };

const initialCounts: Counts = { team: 0, works: 0, featuredWorks: 0, testimonials: 0, featuredTestimonials: 0 };
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const syncStats = async () => {
      try {
        const [teamData, worksData, testimonialsData] = await Promise.all([
          fetchTeamMembers(), fetchWorkItems(), fetchTestimonialItems(),
        ]);
        setCounts({
          team: teamData.length,
          works: worksData.length,
          featuredWorks: worksData.filter((w) => w.featuredOnHome).slice(0, 4).length,
          testimonials: testimonialsData.length,
          featuredTestimonials: testimonialsData.filter((t) => t.featuredOnHome).slice(0, 4).length,
        });
      } finally {
        setIsLoading(false);
      }
    ...
    syncStats();
    return subscribeToContentUpdates(syncStats);
  }, []);
  ...
  const derived = useMemo(() => {
    ...
    const showcaseCoverage = Math.min(100, Math.round((totalHomepagePicks / Math.max(1, counts.works + counts.testimonials)) * 100));
    return { totalHomepagePicks, totalManagedEntries, showcaseCoverage };
  }, [counts]);

  const { totalHomepagePicks, totalManagedEntries, showcaseCoverage } = derived;

  const statCards = [
    { label: 'Team Profiles', value: counts.team, sub: 'Active members', icon: Users, color: 'blue' as ColorKey },
    { label: 'Portfolio Items', value: counts.works, sub: 'Live projects', icon: Briefcase, color: 'green' as ColorKey },
    { label: 'Homepage Picks', value: totalHomepagePicks, sub: `${showcaseCoverage}% coverage`, icon: Star, color: 'purple' as ColorKey },
    { label: 'Testimonials', value: counts.testimonials, sub: `${counts.featuredTestimonials} featured`, icon: MessageSquare, color: 'orange' as ColorKey },
  ];

  const recentActivity = [
    { icon: Activity, text: 'Content synced across all 3 collections', time: 'Just now', color: 'green' as ColorKey },
    { icon: Briefcase, text: `${counts.featuredWorks} works featured on homepage`, time: 'Auto', color: 'blue' as ColorKey },
    { icon: MessageSquare, text: `${counts.featuredTestimonials} testimonials spotlighted`, time: 'Auto', color: 'purple' as ColorKey },
    { icon: Users, text: `${counts.team} team profiles active`, time: 'Auto', color: 'orange' as ColorKey },
  ];

  const quickActions = [
    { label: 'Manage Team', desc: `${counts.team} members`, link: '/admin/team', color: 'blue' as ColorKey, icon: Users },
    { label: 'Curate Portfolio', desc: `${counts.works} projects`, link: '/admin/works', color: 'green' as ColorKey, icon: Briefcase },
  ];

  const healthChecks = [
    { label: 'Homepage work rotation', state: counts.featuredWorks >= 4 ? 'Healthy' : 'Needs focus', link: '/admin/works' },
    { label: 'Trust section coverage', state: counts.featuredTestimonials >= 3 ? 'Healthy' : 'Needs focus', link: '/admin/testimonials' },
    { label: 'Team presence', state: counts.team >= 4 ? 'Healthy' : 'Building', link: '/admin/team' },
  ];
