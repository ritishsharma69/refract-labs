...
const MagnifiedBento = () => {
  ...
  return (
    <div className="flex items-center justify-center p-4 sm:p-6 w-full not-prose">
      <div className="group relative w-full max-w-[420px] overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 bg-[#111] p-1.5 sm:p-2 shadow-2xl shadow-orange-500/5 transition-all duration-500 hover:shadow-orange-500/10 hover:-translate-y-1">
        <div
          ref={containerRef}
          className={cn(
            "relative w-full overflow-hidden rounded-[1.6rem] sm:rounded-[2rem] bg-[#0a0a0a]",
        ...
        >
              ...
              {TAG_ROWS.map((row, rowIndex) => (
                ...
                >
                  {[...row, ...row, ...row].map((item, idx) => (
                    <div
                      key={`${item.id}-${idx}`}
                      className="flex gap-2 bg-white/5 backdrop-blur-sm whitespace-nowrap w-fit text-[#888] p-2 px-3 items-center border border-white/10 rounded-full text-xs"
                    >
                ...
                >
                  {[...row, ...row, ...row].map((item, idx) => (
                    <div
                      ...
                      className="flex gap-2 bg-[#1a1a1a] whitespace-nowrap w-fit text-white p-2 px-3 items-center border border-orange-500/20 shadow-sm rounded-full text-xs scale-125 ml-6"
                    >
                      <HugeiconsIcon icon={item.icon} size={14} className="text-orange-400" />
                      <span className="font-medium text-orange-400">{item.label}</span>
                    </div>
                  ))}
                </motion.div>
              ))}
          ...
          <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-[#0a0a0a] to-transparent z-20"></div>
          <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-[#0a0a0a] to-transparent z-20"></div>
        </div>
